...
def get_options(correct_word, all_words):
    """Doğru kelimeyi ve rastgele diğer 4 kelimeyi seçer ve karıştırır."""
    options = random.sample(all_words, 4)  # 4 rastgele kelime seç
    if correct_word not in options:
        options.append(correct_word)  # Doğru kelimeyi ekle
    random.shuffle(options)  # Seçenekleri karıştır
    return options

# Skor tutucu (oturumda saklanır)
...
if st.button("Cevabı Kontrol Et"):
    if user_choice == kelime:
        st.success(f"Tebrikler! Doğru cevap: {kelime}")
        st.session_state.score += 1  # Skoru artır
    else:
        st.error(f"Yanlış cevap! Doğru cevap: {kelime}")
        st.session_state.score -= 1  # Skoru azalt
